...
namespace Jetpack2.InputWatchers
{
    ...
    public class GazeBuffer
    {
        ...
        {
            float MIN = UIModOptions.GazeBuffer_GazeTarget_RadiiForSpeed_Min;                       // Minimum base radius
            float SPEED_RATIO = UIModOptions.GazeBuffer_GazeTarget_RadiiForSpeed_SpeedRatio;        // Speed to base radius scaling factor
            float MULT = UIModOptions.GazeBuffer_GazeTarget_RadiiForSpeed_StepMult;                 // Multiplier for step progression
        ...
        private static float CalculateDirectionConfidence(List<GazeSample_Direct> samples)
        {
            ...
            if (time_percent < UIModOptions.GazeBuffer_GazeConfidence_Direct)
                return 0f;
        ...
        private static float CalculateDirectionConfidence(List<GazeSample_SphereTarget> samples, Vector3 pos)
        {
            ...
            if (time_percent < UIModOptions.GazeBuffer_GazeConfidence_Target)        // even if all the hits are perfectly aligned, the low amount of time they are around won't make it worth calculating
                return 0f;
            ...
            float normalized_stddev = Mathf.Exp(-std_dev * UIModOptions.GazeBuffer_Confidence_StdDev_DecayMult); // Aggressive drop for std_dev > 0.001 (even a value of 12 is pretty aggressive - add a slider for this)
            ...
            //Debug.Log($"final: {confidence * time_percent}, confidence: {confidence}, time_percent: {time_percent}, std_dev: {std_dev}, avg_confidence: {avg_confidence}, normalized_stddev: {normalized_stddev} (decay mult: {UIModOptions.YawToLook_Buffer_Confidence_StdDev_DecayMult})");
        ...
        {
            float max_seconds = UIModOptions.GazeBuffer_MaxSeconds;
